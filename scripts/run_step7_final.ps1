...
try {
    ...
    if ($PreflightOnly) {
        Write-Host '[Step 7 final] Candidate 003 preflight...'
        powershell.exe -NoProfile -ExecutionPolicy Bypass `
            -File scripts\run_step7_candidate003_calibration.ps1 `
            -PreflightOnly
        if ($LASTEXITCODE -ne 0) {
            throw "Candidate 003 preflight failed with exit code $LASTEXITCODE"
        }
        ...
        if (Test-Path -LiteralPath $calibration -PathType Leaf) {
            ...
            Write-Host '[Step 7 final] Formal profile preflight is deferred until Candidate 003 exists.'
        }
        return
    }

    Write-Host '[Step 7 final] Building or verifying Candidate 003...'
    powershell.exe -NoProfile -ExecutionPolicy Bypass `
        -File scripts\run_step7_candidate003_calibration.ps1
    if ($LASTEXITCODE -ne 0) {
        throw "Candidate 003 stopped Step 7 with exit code $LASTEXITCODE"
    }
    ...
    Write-Host '[Step 7 final] PASS: Candidate 003 and all formal profile artifacts verified.'
}
finally {
    Pop-Location
}
